...
(defn sketch-init [sketch]
  (fn [processing]
    ...
    (set! (.-sketch-data processing) (atom {}))
    (set! (.-sketch-data-map processing) {})
...
;; Trigonometry
(defn acos [value]
  (. *processing* (acos value)))

(defn asin [value]
  (. *processing* (asin value)))

(defn atan [value]
  (. *processing* (atan value)))

(defn  atan2 [y x]
  (. *processing* (atan2 y x)))

(defn cos [angle]
  (. *processing* (cos angle)))

(defn degrees [angle]
  (. *processing (degrees angle)))

(defn radians [angle]
  (. *processing (radians angle)))

(defn sin [radians]
  (. *processing* (sin radians)))

(defn tan [radians]
  (. *processing* (tan radians)))
...
(defn noise
  ([x]
     (. *processing* (noise x)))
  ([x y]
     (. *processing* (noise x y)))
  ([x y z]
     (. *processing* (noise x y z))))

(defn noise-detail
  ([octave]
     (. *processing* (noiseDetail octave)))
  ([octave falloff]
     (. *processing* (noiseDetail octave falloff))))

(defn noise-seed [value]
  (. *processing* (noiseSeed value)))

(defn random-seed [value]
  (. *processing* (randomSeed value)))
